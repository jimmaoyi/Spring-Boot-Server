package com.maryun.lb.restful.jyb.activeUserRelate;
...
import com.maryun.lb.mapper.jyb.activeUserRelate.ActiveUserRelateMapper;
import com.maryun.model.PageData;
import com.maryun.restful.base.BaseRestful;
import com.maryun.utils.WebResult;

/**
 * 类名称：ActiveUserRelateRestful 创建人：ChuMingZe 创建时间：2017年2月26日
 * 
 * @version
 */
@RestController
@RequestMapping(value = "/actUserRela")
public class ActiveUserRelateRestful extends BaseRestful {
	@Autowired
	private ActiveUserRelateMapper activeUserRelateMapper;
	...
	public PageData pageSearch(@RequestBody PageData pd) throws Exception {
		...
		lists = activeUserRelateMapper.listPage(pd);
		// 分页
	...
	public PageData pageSearchByActHis(@RequestBody PageData pd) throws Exception {
		...
		lists = activeUserRelateMapper.listPageByActHis(pd);
		// 分页
		pd = this.getPagingPd(lists);
		// 结果集封装
		return WebResult.requestSuccess(pd);
	...
	public PageData toSelect(@RequestBody PageData pd) throws Exception {
		...
		if (null != s_IDS && !"".equals(s_IDS)) {
			...
			activeUserRelateMapper.updateState(pd);
			pd.put("msg", "success");
		} else {
			pd.put("msg", "failed");
		}
	...
	public PageData getActiveAndAppleInfo(@RequestBody PageData pd) throws Exception {
		pd = activeUserRelateMapper.getActiveAndAppleInfo(pd);
		return WebResult.requestSuccess(pd);
	}
}
